<?php
/**
 * Message translations.
 *
 * This file is automatically generated by 'yiic message' command.
 * It contains the localizable messages extracted from source code.
 * You may modify this file by translating the extracted messages.
 *
 * Each array element represents the translation (value) of a message (key).
 * If the value is empty, the message is considered as not translated.
 * Messages that no longer need translation will have their translations
 * enclosed between a pair of '@@' marks.
 *
 * Message string can be used with plural forms format. Check i18n section
 * of the guide for details.
 *
 * NOTE, this file must be saved in UTF-8 encoding.
 */
return array (
  '"List" command output' => '"Vis" kommando output',
  '% of assigned RAM' => '% af tildelt RAM',
  '% of total system RAM' => '% af system RAM',
  '0 to disable' => 'Intet at deaktivere',
  '<b>.conf file</b>: Contains server binary specific configuration and download links' => '<b>.conf filen</b>: Indeholder server binær spesifik konfiguration og download links',
  '<b>JAR File</b>: The server binary itself' => '<b>JAR filen</b>: Server binær filen',
  'AJAX update interval' => 'AJAX opdatereings interval',
  'Action' => 'Handling',
  'Action failed for daemons:' => 'Handling fejlede for daemons:',
  'Action failed for servers:' => 'Handling fejlede for servere:',
  'Action run for daemons:' => 'Handling kørt på daemon:',
  'Action run for servers:' => 'Handling kørt på servere:',
  'Active Servers' => 'Aktive servere:',
  'Add' => 'Tilføj',
  'Add Files' => 'TIlføj filer',
  'Add or Remove Files' => 'Tilføj eller fjern filer',
  'Administrator Name' => 'Adminstartor navn',
  'Administrator contact Email' => 'Adminstartor email',
  'All Daemons' => 'Alle daemons',
  'All interfaces (0.0.0.0)' => 'Alle interfaces (0.0.0.0)',
  'Allow GET requests to the API' => 'Tillad GET forespørgelser til API',
  'Allow IP change for login sessions by default' => 'Tillad IP ændringer for login som standard',
  'Allow Users to Create a MySQL Database' => 'Tillad brugere at oprette MySQL dtabaser',
  'Allow users to generate API keys' => 'Tillad brugere at oprette API nøgler',
  'Allowing IP changes means that the login session will be considered valid even if a users IP changes. This is useful when logging in on mobile devices but it also means that stolen session cookies will be valid.' => 'At tillade IP ændringer betyder, at login-session vil anses for gyldige, selv om en brugers IP ændres. Dette er nyttigt, når du logger ind på mobile enheder, men det betyder også, at stjålne sessions cookies vil være gyldige',
  'Always Save' => 'Gem altid',
  'Anonymous usage statistics' => 'Anonyme brugs statistikker',
  'Any console output' => 'Ethvert konsol output',
  'Assign Mail' => 'Tildel e-mail',
  'Assume Minecraft is still running on:' => 'Tro at Minecraft stadig køre på_',
  'Auto-save JAR change' => 'Auto gem JAR ændringer',
  'Automatically accept' => 'Automatisk acepter',
  'Automatically save server form on JAR file change' => 'Automatisk gem server formen på JAR fil ændringen',
  'Autosave Interval (0 to disable)' => 'Auto gem interval (0 for at slå fra)',
  'Average players per server' => 'Gennemsnitlig antal spillere per server',
  'Average servers per daemon' => 'Gennemsnitlig antal spillere per daemon',
  'Back' => 'Tilbage',
  'Base server port to use on new IPs' => 'Base server port til brug på nye servere',
  'Block special characters in chat' => 'Bloker special tegn i vhatten',
  'Broadcast' => 'Send',
  'CPU Usage to Display' => 'CPU brug til at vise',
  'Chat' => 'Chat',
  'Check for Multicraft updates' => 'Se efter Multicraft opdateringer',
  'Cleanup' => 'Ryd op',
  'Cleanup Players Table' => 'Ryd op i spiller tabelen',
  'Clear' => 'Ryd',
  'Clear Command Cache' => 'Ryd kommando cahcen',
  'Cleared command cache table.' => 'Slettede kommand cachen',
  'Command successfully sent.' => 'Kommandoen blev sendt',
  'Conf URL' => 'Konf URL',
  'Config File' => 'Konfigurationsfil',
  'Config File Settings' => 'Konfigurationsfil indstillinger',
  'Console' => 'Konsol',
  'Crashed Server Restart Delay (0 to disable)' => 'Crashede server genstarts dforsinkelse (0 for at slå fra)',
  'Daemon' => 'Daemon',
  'Daemon FTP Server IP' => 'Daemon FTP server IP',
  'Daemon IP' => 'Daemon IP',
  'Daemon Operations' => 'Daemon operationer',
  'Daemons' => 'Daemons',
  'Default' => 'Standard',
  'Default Display IP' => 'Standard vist IP',
  'Default amount of memory' => 'Standar mængde hukommelse (RAM)',
  'Default amount of startup memory' => 'Standars mængde start hukommelse',
  'Default number of player slots' => 'Standard antal spiller',
  'Default server IP' => 'Standard  server IP',
  'Default server name' => 'Standard server navm',
  'Deleted {del} player entries.' => 'Slettede {del} spiller genstande',
  'Deletes all players that have the same role as the Default Role of their server' => '',
  'Disable AJAX updates' => '',
  'Disable User Registration' => '',
  'Disable template setup functionality' => '',
  'Disable the integrated FTP client (net2ftp)' => '',
  'Disables the user registration functionality' => '',
  'Display server memory to users in the advanced section' => '',
  'Download only' => '',
  'Downloading {percent}' => '',
  'Enable CSRF validation' => '',
  'Enable Cookie validation' => '',
  'Enable IP Authentication' => '',
  'Enable the Multicraft API' => '',
  'Error, please check daemon connection' => '',
  'Everyone' => '',
  'Execute' => '',
  'Failed to save settings.' => '',
  'File' => '',
  'File URL' => '',
  'For example a link to phpMyAdmin. "*" will be replaced with the daemon IP' => '',
  'Function disabled in demo mode.' => '',
  'Generate server status banners (requires GD)' => '',
  'Global Operations' => '',
  'Go' => '',
  'Hide Advanced Options' => '',
  'Hide the userlist from normal users' => '',
  'Install' => '',
  'Invalid administrator email set under Settings->Panel Configuration' => '',
  'Invalid file name specified.' => '',
  'JAR File' => '',
  'JAR Filename' => '',
  'JAR and Config' => '',
  'Known console output' => '',
  'Language' => '',
  'Limit number of console lines per second to' => '',
  'Live Statistics' => '',
  'Logged in Users' => '',
  'Login block interval' => '',
  'MB' => '',
  'Manual' => '',
  'Memory' => '',
  'Memory Assigned' => '',
  'Message' => '',
  'Minecraft Crash Check Interval' => '',
  'Minecraft EULA' => '',
  'Minecraft Manager Settings' => '',
  'Minecraft Response Timeout (0 to disable)' => '',
  'Minimum password length' => '',
  'Minimum time between two resource checks (0 to disable)' => '',
  'Minimum time between two world backups' => '',
  'Minimum time between two world saves' => '',
  'Mobile Theme' => '',
  'Multicraft Operations' => '',
  'Multicraft Statistics' => '',
  'Multicraft Status' => '',
  'Must have database create and grant privileges' => '',
  'Never Save' => '',
  'New JAR setting for servers using the removed JAR' => '',
  'No' => '',
  'No command to send.' => '',
  'No daemons affected' => '',
  'No daemons found' => '',
  'No daemons found.' => '',
  'No message to send.' => '',
  'No one' => '',
  'No servers affected' => '',
  'No update in progress' => '',
  'Not recommended. If enabled, consider disabling user registration' => '',
  'Number of Ping Timeouts to Allow' => '',
  'Number of backups to keep' => '',
  'Number of lines to store in chat window' => '',
  'Number of lines to store in log/console window' => '',
  'Number of login attempts before blocking' => '',
  'Number of servers to display at once on the serverlist' => '',
  'Of Total' => '',
  'Online Players' => '',
  'Online players' => '',
  'Online servers' => '',
  'Only allow these IPs to login as a superuser, empty for no restriction. This will also restrict superuser access through the API.' => '',
  'Only allow these IPs to use the API, empty for no restriction.' => '',
  'Only check for updates as Superuser' => '',
  'Operations' => '',
  'Panel Configuration' => '',
  'Password Encryption' => '',
  'Please check that at least one daemon is started and that it uses the same database you configured as the daemon database using the control panel installer.' => '',
  'Please choose a file type to delete.' => '',
  'Please choose a file type to download.' => '',
  'Plugin repository refresh interval' => '',
  'RAM Usage to Display' => '',
  'Ready for installation' => '',
  'Reduces HTTP requests but disables console/chat/status autorefresh' => '',
  'Remove' => '',
  'Remove Files' => '',
  'Requires the PDO SQLite extension' => '',
  'Reset Token Valid for (hours)' => '',
  'Restart' => '',
  'Restrict API IPs' => '',
  'Restrict Administrator IPs' => '',
  'Resume' => '',
  'Running Servers' => '',
  'Save' => '',
  'Save player information (ip, lastseen, etc.)' => '',
  'See the How-To section on the Multicraft website' => '',
  'Send a notification email when a server is assigned to a user' => '',
  'Send a welcome email when a new user is created' => '',
  'Sender' => '',
  'Server Restart Delay' => '',
  'Servers' => '',
  'Set to * to use the daemon IP' => '',
  'Setting' => '',
  'Settings' => '',
  'Show "Accept EULA" button' => '',
  'Show "Delete all Players" link for' => '',
  'Show Advanced Options' => '',
  'Show Server List For' => '',
  'Show Server Memory' => '',
  'Show the kill button for' => '',
  'Show the repair tool button for' => '',
  'Start' => '',
  'Statistics' => '',
  'Status' => '',
  'Stop' => '',
  'Superusers' => '',
  'Suspend' => '',
  'Suspended Servers' => '',
  'The reset token expires after the specified number of hours. Use 0 to disable the password reset functionality.' => '',
  'The target filename should always be of the format "example.jar". The downloaded files will automatically be named "example.jar" and "example.jar.conf" no matter how they were named in their original location. To skip one of the files you can simply leave the URL field empty.' => '',
  'The time between two AJAX updates (for example console refresh) in milliseconds. Minimum 250, default 2000.' => '',
  'The user database is named prefix + server ID' => '',
  'Theme' => '',
  'This clears the table that caches daemon queries, Proceed?' => '',
  'This forcefully ends the server, potentially resulting in corruption.' => '',
  'This requires more configuration, see the How-To section on the Multicraft website.' => '',
  'This will cause a separate HTTP request for each server visible on the list' => '',
  'This will only remove the daemon entry for this server from the database. If the daemon is still running or is started again this entry will be put back in.' => '',
  'Timeout for client communication' => '',
  'Total Servers' => '',
  'Total memory' => '',
  'Total memory assigned' => '',
  'Total memory assigned to online servers' => '',
  'Total player slots' => '',
  'Type' => '',
  'Unknown action "{action}"' => '',
  'Up to date' => '',
  'Update' => '',
  'Update Existing' => '',
  'Update Minecraft' => '',
  'Update Settings' => '',
  'Update successful' => '',
  'Usage in MB' => '',
  'Usage in MB (Capped)' => '',
  'Usage of entire CPU' => '',
  'Usage of one core' => '',
  'Use AJAX in the serverlist for faster loading' => '',
  'Use BukGet plugin list' => '',
  'Use schema cache to reduce queries' => '',
  'Use separate DB for command cache' => '',
  'Used' => '',
  'Used when a new server is created' => '',
  'User DB Admin Link' => '',
  'User DB Host' => '',
  'User DB Password' => '',
  'User DB Prefix' => '',
  'User DB Username' => '',
  'Users' => '',
  'Values in brackets include suspended servers.' => '',
  'Version: {version} (latest: {remote})' => '',
  'Warning: This will affect all servers running on the selected daemon(s)!

Selected action: ' => '',
  'Warning: This will delete all players that haven\'t been explicitly assigned a role. This will be the vast majority of player entries and the "last seen", "quit reason" and "IPs" information for these players will be lost. Proceed?' => '',
  'Welcome Mail' => '',
  'When you want to delete the "example.jar.conf" you can enter "example.jar" as the file to delete and then select ".conf File" in the drop down menu. Seleting "Both Files" will delete both the "example.jar" and the "example.jar.conf".' => '',
  'Yes' => '',
  'Your current panel version is {v}' => '',
  '[x]' => '',
  'empty to hide the "Support" menu entry' => '',
  'false' => '',
  'in seconds' => '',
  'true' => '',
);
